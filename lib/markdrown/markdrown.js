export class Markdrown {
    ...
    parse(md, wrapSections = true) {
        ...
        const inlineParse = (inputLines) => {
            ...
            // Protect inline code spans from all other inline formatting.
            // Without this, emphasis (priority 10) runs before the inline-code
            // plugin (priority 30) and mangles underscores/asterisks inside
            // backticks, e.g. `OTEL_EXPORTER_OTLP_ENDPOINT` -> OTEL<em>EXPORTER</em>...
            const codeStore = [];
            html = html.replace(/`([^`]+?)`/g, (m, a) => {
                const token = ` CODE${codeStore.length} `;
                codeStore.push(`<code>${Markdrown.escapeHTML(a)}</code>`);
                return token;
            });
            ...
            html = html.replace(/ CODE(\d+) /g, (m, idx) => codeStore[Number(idx)]);
            return html;
        };
...
function linkPlugin() {
    return {
        type: 'inline',
        priority: 40,
        // Do NOT escape the link text: earlier inline plugins (inline code,
        // strong/em) may have already turned it into rendered HTML. Escaping
        // here would double-escape that markup (e.g. `<code>` -> `&lt;code&gt;`).
        // Raw plain text is left unescaped to match the rest of the parser.
        parse: text => text.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (m, text, url) =>
            `<a href="${Markdrown.escapeHTML(url)}">${text}</a>`
        )
    };
}
...
function tablePlugin() {
    return {
        ...
        parse: (lines, i, inline) => {
            ...
            html += '</tbody></table>';
            // Wrap in a scroll container so wide tables scroll horizontally
            // instead of overflowing/clipping on narrow viewports.
            html = `<div class="md-table-wrap">${html}</div>`;
